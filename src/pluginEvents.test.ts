...
const mocks = vi.hoisted(() => ({
  ...
  collectSessionStartMessages: vi.fn(() => Promise.resolve([] as string[])),
}))

vi.mock("bitfab-plugin-lib", async (importOriginal) => ({
  ...(await importOriginal<typeof import("bitfab-plugin-lib")>()),
  ...mocks,
}))

type Handler = (event: unknown, ctx?: unknown) => unknown

async function registeredHandlers(): Promise<Map<string, Handler>> {
  vi.resetModules()
...
describe("Amp event handlers", () => {
  it("waits for the turn's upload before the turn ends", async () => {
    ...
    mocks.captureAmpAgentEnd.mockImplementationOnce(
      () =>
    ...
    const pending = Promise.resolve(
      agentEnd?.({ thread: { id: "T-1" }, messages: [] }),
    ).then(() => {
      settled = true
    })
...
describe("session start notices", () => {
  it("shows the update notice once, not on every thread switch", async () => {
    ...
    const notify = vi.fn((_message: string) => Promise.resolve())
    const handlers = await registeredHandlers()
