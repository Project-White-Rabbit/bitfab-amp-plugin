---
name: update
description: "Update Bitfab plugin and SDK to the latest versions. TRIGGER when: user wants to update Bitfab, upgrade the SDK, get the latest version, or says 'update bitfab', 'upgrade SDK', 'latest version'. SKIP when: user wants to instrument code or iterate on traces. Invoke as bitfab:update [all|plugin|sdk]."
---

# Bitfab Update

Update the Bitfab Amp plugin and/or every workspace's SDK in the current project.
...
Read `the user's request` first. If its first token is exactly one of the mode names below, run that mode. Otherwise, when this skill documents how to route the remaining arguments (see its intro), follow that; if it doesn't, run `all` and treat `the user's request` as its input. Run only that mode's section below and skip the others.
...
0. Amp does not inject a plugin-root env var. Resolve it first (same block used by `setup` and `assistant`):
...
1. If the plugin was updated, remind the user to restart Amp to apply the update. If the mode was `plugin`, stop here; do not run any step of the SDK phase.
