...
2. Call `list_organizations` to list the organizations the signed-in user belongs to. Each entry has a name, the user's role, an `id:` (the `authOrganizationId`), and the org the plugin uses now is marked `[current]`.
