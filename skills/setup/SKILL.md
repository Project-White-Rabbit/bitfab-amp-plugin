...
2. The user asked to be walked through it. **Before anything else** (before dispatching to the *Instrument* section, before a single probe or file read), render the content of the block below **verbatim** as formatted markdown: no code fence, no rewording, no additions.
   ...
   **What's about to happen next**
   ...
   - This wizard will guide Amp on how to use the Bitfab plugin to analyze your repository. Amp will then instrument your AI features and register their production entrypoints in a replay registry using the Bitfab SDK.
...
7. Each edit is driven by the user’s request in chat. Include traceFunctionKey: <key> in every template read and save. Ask which span or region they mean when unclear.
   ...
   5. Call `save_template` with `spanType`, `traceFunctionKey: <key>`, and the full edited body. The tool upserts the per-function row in place (no version bump, no row juggling). On the first save for a span type the row is created; subsequent edits update it. The preview updates when the save completes.
